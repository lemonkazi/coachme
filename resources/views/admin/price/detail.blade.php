        ...
        <div class="col-12">
          <div class="card card-primary">
              ...
              foreach ($result as $key => $value) {
                ...
                if (!empty($data['price']->$key)) {
                    if (!empty($result[$key]['is_date_format'])) {
                        $result[$key]['text'] = date('Y年m月d日', $data['price']->$key);
                    } elseif (!empty($result[$key]['is_array'])) {
                        $result[$key]['text'] = implode (", ", $data['price']->$key);
                    } elseif (!empty($result[$key]['related'])) {
                      $related = (string)$result[$key]['related'];
                      $result[$key]['text'] = $data['price']->$related->name;
                    } elseif ($key == "avatar_image_path") {
                      $result[$key]['text'] = "<img style='max-width:80px;' src='{$BASE_URL}/photo/price_photo/{$data['price']->$key}' />";
                    } else {
                        $result[$key]['text'] = $data['price']->$key;
                    }
                } else {
                    $result[$key]['text'] = '-';
                }
            ...
            <div class="card-footer text-center">
                ...
                <a class="col-sm-2 btn bg-gradient-primary btn-sm text-center" href="<?php echo $BASE_URL . '/price/edit/' . $data['price']->id ?>"><?php echo trans('global.LABEL_UPDATE') ?></a>
